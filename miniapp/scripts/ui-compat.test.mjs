...
assert.ok(bookingSessionGuardBody !== undefined, 'appointment records should centralize current-token session checks')
assert.match(bookingSessionGuardBody, /ticket !== loadTicket/, 'appointment records should reject an old load generation')
assert.match(bookingSessionGuardBody, /token === currentToken/, 'appointment records should accept responses only for the current token')
assert.match(bookingSessionGuardBody, /!currentToken[\s\S]*error\?\.authExpired[\s\S]*error\.requestToken === token/, 'appointment records should recognize a current-session 401 after request cleanup')
const staleBookingBody = vueFunctionBody(bookingRecordsPage, 'invalidateStaleBookingSession')
assert.ok(staleBookingBody !== undefined, 'appointment records should isolate stale-token cleanup from auth redirect')
assert.match(staleBookingBody, /bookings\.value\s*=\s*\[\]/, 'stale-token cleanup should remove the old user booking list')
assert.match(staleBookingBody, /clearBookingSession\(\)/, 'stale-token cleanup should remove the old booking session')
assert.doesNotMatch(staleBookingBody, /clearToken|showToast|switchTab|navigateTo/, 'stale-token cleanup must not mutate or redirect the newer auth session')
const bookingLoadBody = vueFunctionBody(bookingRecordsPage, 'loadBookings')
assert.match(bookingLoadBody, /if \(!isCurrentBookingSession\(ticket, requestToken\)\)[\s\S]*invalidateStaleBookingSession\(ticket\)[\s\S]*loadedToken = requestToken/, 'appointment success should reject an old token before exposing records')
assert.match(bookingLoadBody, /catch \(error\) \{\s*if \(!isCurrentBookingSession\(ticket, requestToken, error\)\)[\s\S]*invalidateStaleBookingSession\(ticket\)[\s\S]*if \(isAuthError\(error\)\)/, 'appointment failures should reject an old token before auth side effects')
const openBookingBody = vueFunctionBody(bookingRecordsPage, 'openBooking')
assert.match(openBookingBody, /if \(currentToken !== loadedToken\) \{\s*invalidateStaleBookingSession\(\)\s*return/, 'clicking an old-token record should invalidate it without auth redirect')
