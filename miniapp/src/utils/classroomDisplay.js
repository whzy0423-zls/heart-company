...
export function normalizeClassroomSeries(source = {}) {
  ...
  return {
    id: id(source.id), title: text(source.title), summary: text(source.summary), coverUrl: text(source.coverUrl),
    teacherName: text(source.teacherName), effectiveAccess, priceCents: nonNegativeInteger(source.priceCents), canPlay,
...
export function classroomAccessLabel(value) {
  return ({ public: '免费', login: '登录可学', member: '会员专享', paid: '付费课件' })[value] || '权限待确认'
}

export function classroomPurchaseAction(item = {}) {
  if (item.playbackBlocked) return { type: 'blocked', label: '暂不可播放' }
  if (item.canPlay) return { type: 'play', label: item.purchaseState === 'owned' ? '继续学习' : '立即学习' }
