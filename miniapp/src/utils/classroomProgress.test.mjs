...
try {
  ...
  const anonymous = createClassroomProgressTracker({
    contentId: 21,
    durationSeconds: 100,
    storage,
  });
  assert.deepEqual(await anonymous.record(25), {
    positionSeconds: 25,
    completed: false,
    local: true,
  });
  assert.deepEqual(await anonymous.record(90), {
    positionSeconds: 90,
    completed: true,
    local: true,
  });
  assert.deepEqual(
    await anonymous.record(40),
    { positionSeconds: 40, completed: true, local: true },
    "completed progress must not regress after seeking backward",
  );
  ...
  assert.deepEqual(
    sent,
    [{ contentId: "22", positionSeconds: 10 }],
    "updates inside the window should be throttled",
  );
  ...
  assert.deepEqual(
    sent.at(-1),
    { contentId: "22", positionSeconds: 40 },
    "pause/unload flush should bypass throttling",
  );
  ...
  assert.deepEqual(
    concurrentSent,
    [10],
    "records during an in-flight write must not start concurrent requests",
  );
  assert.deepEqual(
    concurrent.pending(),
    { positionSeconds: 30, completed: false },
    "only the latest throttled snapshot should remain queued",
  );
  firstSend.resolve();
  await firstRecord;
  ...
  const flushPromise = flushing.flush().then(() => {
    firstFlushDone = true;
  });
  const simultaneousFlush = flushing.flush().then(() => {
  ...
  assert.deepEqual(
    flushSent,
    [10, 40],
    "flush must send the newest queued snapshot after the active request",
  );
  assert.equal(
    firstFlushDone || secondFlushDone,
    false,
    "all simultaneous flush callers must wait for the serialized latest write",
  );
  flushSecond.resolve();
  ...
  assert.deepEqual(
    retrying.pending(),
    { positionSeconds: 60, completed: false },
    "a failed older request must not replace the newest queued snapshot",
  );
  ...
  assert.deepEqual(
    await authoritative.record(95),
    { positionSeconds: 95, completed: false },
    "logged-in completion must wait for the server",
  );
  assert.deepEqual(
    await authoritative.record(96, { force: true }),
    { positionSeconds: 96, completed: true },
    "server completion should update the tracker snapshot",
  );
  assert.deepEqual(
    await authoritative.record(20),
    { positionSeconds: 20, completed: true },
    "server-confirmed completion must stay monotonic",
  );
