...
assert.equal(typeof storage[BOOKING_INTENT_KEY].timestamp, 'number', 'successful storage includes a timestamp')
assert.deepEqual(storage[BOOKING_INTENT_KEY], {
  kind: 'consult',
  intentText: '想了解咨询',
  timestamp: storage[BOOKING_INTENT_KEY].timestamp,
...
throwSet = false
storage[BOOKING_INTENT_KEY] = { kind: 'consult', intentText: 'read fails', timestamp: 1 }
throwGet = true
assert.equal(consumeBookingIntent(), null, 'read failures return null')
throwGet = false
assert.equal(storage[BOOKING_INTENT_KEY], undefined, 'read failure still attempts cleanup')
storage[BOOKING_INTENT_KEY] = { kind: 'consult', intentText: 'remove fails', timestamp: 1 }
throwRemove = true
...
throwRemove = false
assert.ok(removedKeys.filter((key) => key === BOOKING_INTENT_KEY).length >= 1, 'consumption attempts removal')
