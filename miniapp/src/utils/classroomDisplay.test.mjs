...
try {
  ...
  const series = normalizeClassroomSeries({ id: 12, title: ' 入门系列 ', effectiveAccess: 'member', canPlay: true })
  ...
  assert.deepEqual(classroomPurchaseAction({ canPlay: true, purchaseState: 'owned' }), { type: 'play', label: '继续学习' })
  assert.deepEqual(classroomPurchaseAction({ canPlay: false, purchaseState: 'purchase_required', priceCents: 990 }), { type: 'purchase', label: '¥9.90 购买' })
  assert.deepEqual(classroomPurchaseAction({ canPlay: false, effectiveAccess: 'login' }), { type: 'login', label: '登录后学习' })
  assert.deepEqual(classroomPurchaseAction({ canPlay: false, effectiveAccess: 'member' }), { type: 'member', label: '开通会员' })
  assert.deepEqual(classroomPurchaseAction({ playbackBlocked: true }), { type: 'blocked', label: '暂不可播放' })

  assert.equal(classroomContentRoute({ id: 21, contentType: 'video' }), '/pages/classroom-detail/classroom-detail?id=21&type=video')
  assert.equal(classroomContentRoute({ id: 22, contentType: 'audio' }), '/pages/classroom-detail/classroom-detail?id=22&type=audio')
