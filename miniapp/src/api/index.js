...
export function listClassroomSeriesApi(query = {}) {
  return optionalClassroomRequest(
    { url: "/public/classroom/series", method: "GET", query },
    "系列课程加载失败，请重试",
  );
}
...
export function listClassroomStandaloneApi(query = {}) {
  return optionalClassroomRequest(
    { url: "/public/classroom/standalone", method: "GET", query },
    "课件列表加载失败，请重试",
  );
}
...
export function getClassroomContentApi(id) {
  return optionalClassroomRequest(
    { url: `/public/classroom/content/${classroomID(id)}`, method: "GET" },
    "课件详情加载失败，请重试",
  );
}
...
export async function getClassroomPlaybackApi(id) {
  ...
  const anonymousPlayback = async () => {
    const result = await classroomRequest(
      { url: `/public/classroom/content/${contentId}/ticket`, method: "POST", data: {} },
      "播放凭证获取失败，请重试",
    );
...
function isExpiredPlaybackError(error) {
  ...
  return (
    code.includes("expired") || message.includes("expiredtoken") || message.includes("url expired")
  );
}
  ...
  return classroomRequest(
    {
      ...
      method: "POST",
      data: { targetType, refId: classroomID(refId) },
      auth: true,
    },
    "课堂订单创建失败，请重试",
  );
  ...
  return classroomRequest(
    {
      ...
      method: "GET",
      query: { targetType, refId: classroomID(refId) },
      auth: true,
    },
    "订单状态查询失败，请重试",
  );
