...
describe('xinzhili model voice option reuse', () => {
  it('reuses the existing voice option library for cloned voices', () => {
    expect(voiceApiSource).toContain('getVoiceOptionsApi');
    expect(voiceApiSource).toContain("'/voice/options'");

    expect(viewSource).toContain('getVoiceOptionsApi');
    expect(viewSource).toContain('ttsVoiceOptions');
    expect(viewSource).toContain('可直接复用声音管理里的已克隆音色');
    expect(viewSource).toContain('选择已有音色');
    expect(viewSource).toContain('handleTtsVoiceOptionChange');
    expect(viewSource).toContain("item.source === 'clone'");
