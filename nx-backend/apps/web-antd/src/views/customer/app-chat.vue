...
import { Button, Card, Descriptions, Drawer, Input, Select, Space, Table, Tag } from 'ant-design-vue';
...
async function load() { loading.value = true; try { const r = await getAppChatAuditMessagesApi({ ...query, keyword: query.keyword || undefined, role: query.role || undefined, feedback: query.feedback || undefined }); items.value = r.items; total.value = r.total; } finally { loading.value = false; } }
function search() { query.page = 1; load(); }
function change(p: { current?: number; pageSize?: number }) { query.page = p.current ?? 1; query.pageSize = p.pageSize ?? 20; load(); }
