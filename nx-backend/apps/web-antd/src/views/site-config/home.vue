...
function addAction() {
  config.value?.home.hero.actions.push({
    label: '新按钮',
    to: '/',
    type: 'route',
    variant: 'ghost',
...
<template>
  <EditorShell
    description="配置首页 Hero、老师简介 teaser、小游戏入口和九型概览标题。"
    :loading="loading"
    :saving="saving"
    title="首页管理"
    @save="saveConfig"
  >
    <Form v-if="config" layout="vertical">
      ...
      <Card
        v-for="(item, index) in config.home.hero.actions"
        :key="index"
        size="small"
      >
        <Row :gutter="12">
          <Col :md="6" :xs="24">
            <Input v-model:value="item.label" placeholder="文字" />
          </Col>
          <Col :md="7" :xs="24">
          ...
          <Col :md="3" :xs="24">
            <Button
              danger
              block
              @click="removeAt(config.home.hero.actions, index)"
            >
              删除
            </Button>
          </Col>
      ...
      <Card
        v-for="(item, index) in config.home.hero.stats"
        :key="index"
        size="small"
      >
        <Row :gutter="12">
          <Col :md="6" :xs="24">
            <Input v-model:value="item.value" placeholder="数值" />
          </Col>
          <Col :md="6" :xs="24">
            <Input v-model:value="item.suffix" placeholder="后缀" />
          </Col>
          <Col :md="9" :xs="24">
          ...
          <Col :md="3" :xs="24">
            <Button
              danger
              block
              @click="removeAt(config.home.hero.stats, index)"
            >
              删除
            </Button>
          </Col>
