...
function shot(id: string, orderNum: number) {
  return {
    actionDescription: `镜头 ${orderNum} 动作`,
    aspectRatio: '16:9',
    cameraMovement: '',
    characterIds: [],
    createTime: '',
    ...
    projectId: '1',
    sceneId: '',
    scriptOriginalContent: `段落 ${orderNum}`,
    selectedGenerationId: '',
    selectedGenerationRevision: 0,
...
function makeEntry(id: string, readiness: string) {
  ...
  if (readiness === 'completed') {
    Object.assign(item.shot, {
      selectedGenerationId: `9${id}`,
      selectedGenerationRevision: 1,
      selectedGenerationStatus: 'completed',
      videoUrl: `https://example.test/${id}.mp4`,
    });
  }
  return item;
...
async function installFixture(page: Page, scenario?: Scenario) {
  ...
  if (
    ...
    state.shots[0].activeSubmission = {
      requestKey: '11111111-1111-4111-8111-111111111111',
      status: scenario === 'reconciled' ? 'reconciled' : 'accepted',
      submissionId: 11,
      taskId: 'task-11',
    };
  }
  ...
  await page.route('http://127.0.0.1:4317/api/**', async (route) => {
    ...
    if (path === '/api/auth/login') data = { accessToken: 'e2e-token' };
    ...
    else if (path === '/api/menu/all')
      data = [
        {
          component: '/video/production/index',
          id: 1008,
          meta: { icon: 'lucide:clapperboard', title: '制片工作台' },
        ...
        {
          component: '/video/projects/index',
          id: 1006,
          meta: { icon: 'lucide:folder-kanban', title: '项目列表' },
          name: 'VideoProjects',
          path: '/video/projects',
          pid: 0,
          status: 1,
        ...
        {
          component: '/video/projects/workflow',
          id: 1007,
          meta: {
          ...
          path: '/video/projects/:id/workbench',
          pid: 0,
          status: 1,
          type: 'menu',
        },
        {
          component: '/video/projects/workbench',
          id: 1010,
          meta: {
            activePath: '/video/projects',
            hideInMenu: true,
          ...
          path: '/video/projects/:id/workbench/advanced',
          pid: 0,
          status: 1,
          type: 'menu',
        },
      ];
    else if (path === '/api/site-config' || path === '/api/system/branding')
      data = {};
    else if (path === '/api/video/projects/list')
      data = {
        items: state.shots.length ? [state.project] : [],
        total: state.shots.length ? 1 : 0,
      };
    else if (path === '/api/video/projects' && request.method() === 'POST')
      data = state.project;
    else if (path === '/api/video/projects/1' && request.method() === 'PUT') {
      ...
      if (
        (scenario === 'accepted' || scenario === 'reconciled') &&
        state.workflowGets >= 2
      ) {
      ...
      if (
        scenario === 'unknown' &&
        state.reconcilePosts &&
        state.workflowGets >= 2
      )
        state.shots[0] = makeEntry('1', 'completed');
      state.project.totalShots = state.shots.length;
      state.project.completedShots = state.shots.filter(
        (item: any) => item.readiness === 'completed',
      ).length;
      ...
      data = {
        ...
        shots: state.shots.map((item: any) => ({
          ...item,
          canGenerate: ['ready', 'stale', 'failed'].includes(item.readiness),
        })),
        steps: {
          brief: state.project.scriptContent
            ? 'complete'
            : state.shots.length
              ? 'skipped_existing'
              : 'blocked',
          assets:
            state.characters.length || state.scenes.length
              ? 'complete'
              : state.shots.length
                ? 'skipped_existing'
                : 'optional',
          storyboard: state.shots.length ? 'complete' : 'blocked',
          generate:
            state.shots.every((item: any) => item.readiness === 'completed') &&
            state.shots.length
              ? 'complete'
              : state.shots.some((item: any) => item.readiness === 'stale')
                ? 'stale'
                : 'blocked',
          export: state.project.finalVideoUrl
            ? scenario === 'stale-final'
              ? 'stale'
              : 'complete'
            : 'blocked',
        },
      ...
      if (scenario === 'partial-import' && !state.partialReturned) {
        ...
        data = {
          created: [
            { index: 0, shotId: '1', sourceKey: 'source-1', status: 'created' },
          ],
          existing: [
            {
              index: 1,
        ...
        const created = body.items.map((item: any, index: number) =>
          makeEntry(String(offset + index + 1), 'ready'),
        );
        state.shots.push(...created);
        data = {
          created: created.map((item: any, index: number) => ({
            index,
            shotId: item.shot.id,
            sourceKey: `source-${item.shot.id}`,
            status: 'created',
          })),
          existing: [],
          failed: [],
      ...
      path.startsWith('/api/video/shots/') &&
      request.method() === 'PUT'
    ) {
      const item = state.shots.find((entry: any) =>
        path.endsWith(`/${entry.shot.id}`),
      );
      Object.assign(item.shot, body);
      data = item.shot;
    } else if (
      path === '/api/video/projects-characters/1' &&
      request.method() === 'GET'
    )
      ...
      const item = { ...body, id: 'c1' };
      state.characters.push(item);
      data = item;
    } else if (
      path === '/api/video/projects-scenes/1' &&
      request.method() === 'GET'
    )
      data = state.scenes;
    else if (
      ...
      const item = { ...body, id: 's1' };
      state.scenes.push(item);
      data = item;
    } else if (path === '/api/video/projects-batch-generate-safe/1') {
      state.generationPosts += 1;
      state.batchShotIds = body.items.map((item: any) => item.shotId);
      for (const item of state.shots.filter((entry: any) =>
        state.batchShotIds.includes(entry.shot.id),
      )) {
        item.readiness = 'ready';
        item.shot.generationId = `9${item.shot.id}`;
      ...
      data = {
        failedCount: 0,
        projectId: '1',
        shotResults: state.batchShotIds.map((shotId: string) => ({
          errorMessage: '',
          generationId: `9${shotId}`,
          orderNum: Number(shotId),
          shotId,
          shotName: `分镜 ${shotId}`,
          status: 'success',
        })),
        successCount: state.batchShotIds.length,
        totalShots: state.batchShotIds.length,
      };
    } else if (path.startsWith('/api/video/shots-video-versions/list/'))
      data = state.versions[path.split('/').at(-1)!] || [];
      ...
      item.readiness = 'completed';
      Object.assign(item.shot, {
        selectedGenerationId: generationId,
        selectedGenerationRevision: 1,
        selectedGenerationStatus: 'completed',
        videoUrl: `https://example.test/${generationId}.mp4`,
      });
      state.versions[shotId] = (state.versions[shotId] || []).map(
        (entry: any) => ({ ...entry, isCurrent: entry.id === generationId }),
      );
      data = item.shot;
    } else if (
      path.startsWith('/api/video/generation-submissions/reconcile/')
    ) {
      state.reconcilePosts += 1;
      state.shots[0].readiness = 'generating';
      state.shots[0].activeSubmission.status = 'reconciled';
      data = {
        generationId: '91',
      ...
      state.composeAttempts += 1;
      state.composePolls = 0;
      data = {
        error: '',
        inputHash: 'hash',
        isCurrent: false,
        jobId: String(7 + state.composeAttempts),
        progress: 0,
        projectId: '1',
        status: 'queued',
        videoUrl: '',
      ...
      const failed =
        scenario === 'compose-failure' && state.composeAttempts === 1;
      const complete = !failed && state.composePolls > 1;
      if (complete) {
        state.project.finalVideoUrl = 'https://example.test/final.mp4';
        state.project.finalVideoInputHash = 'hash';
      }
      data = {
        error: failed ? 'mock compose failure' : '',
        inputHash: 'hash',
        inputSnapshot: {},
        isCurrent: complete,
        jobId: path.split('/').at(-1),
        progress: failed ? 35 : complete ? 100 : 70,
        projectId: '1',
        status: failed ? 'failed' : complete ? 'completed' : 'processing',
        videoUrl: complete ? state.project.finalVideoUrl : '',
      };
    }
    await route.fulfill({
...
async function login(page: Page) {
  ...
  await page.mouse.up();
  await expect(
    page.getByText(/验证通过|验证成功|verification succeeded/i),
  ).toBeVisible();
...
async function openWorkflow(page: Page, step?: string) {
  ...
  await expect(
    page.getByRole('navigation', { name: '视频制作步骤' }),
  ).toBeVisible();
}
...
async function selectEveryVersion(page: Page, count: number) {
  for (let index = 0; index < count; index += 1) {
    if (index > 0)
      await page
        .getByRole('tablist', { name: '分镜生成状态' })
        .getByRole('button', { name: /可生成/ })
        .click();
    await page.getByRole('button', { name: '查看版本' }).first().click();
    await page.getByRole('button', { name: '设为当前' }).click();
    await page.keyboard.press('Escape');
    await expect(
      page.getByRole('button', { name: '查看版本' }).last(),
    ).toBeFocused();
  }
}

test('complete five-step workflow selects versions explicitly and never reaches a real paid gateway', async ({
  page,
}) => {
  ...
  await dialog
    .getByRole('textbox', { name: '参考图 URL' })
    .fill('https://example.test/character.png');
  ...
  await dialog
    .getByRole('textbox', { name: '参考图 URL' })
    .fill('https://example.test/scene.png');
  await dialog.getByRole('button', { name: '确 定' }).click();
  await page
    .getByRole('navigation', { name: '视频制作步骤' })
  ...
  await page.getByRole('button', { name: '保存并继续' }).click();
  await expect(
    page.getByText('免费演练模式：使用本地占位视频，不会调用收费接口。'),
  ).toBeVisible();
  ...
  expect(
    state.shots.every((item: any) => !item.shot.selectedGenerationId),
  ).toBe(true);
...
test('legacy project recommendation and partial import remain resumable', async ({
  page,
}) => {
  await installFixture(page, 'legacy');
  await login(page);
  ...
  await expect(page.getByText('existing 1')).toBeVisible();
...
for (const scenario of ['accepted', 'reconciled'] as const) {
  test(`${scenario} submission resumes polling after reload`, async ({
    page,
  }) => {
    const { state } = await installFixture(page, scenario);
    await login(page);
    await openWorkflow(page, 'generate');
...
test('unknown outcome exposes reconcile-only recovery without a paid repost', async ({
  page,
}, testInfo) => {
  const { state } = await installFixture(page, 'unknown');
  await login(page);
  await openWorkflow(page, 'generate');
  ...
  await page.screenshot({
    path: testInfo.outputPath('guided-workflow-recovery.png'),
    fullPage: true,
  });
});

test('failed latest attempt with valid selection stays completed and stale selection is generatable', async ({
  page,
}) => {
  const { state } = await installFixture(page, 'failed-selection');
  await login(page);
  await openWorkflow(page, 'generate');
  await page
    .getByRole('tablist', { name: '分镜生成状态' })
    ...
    .click();
  await expect(page.getByText('completed')).toBeVisible();
  state.shots = [makeEntry('1', 'stale')];
  await page.reload();
  await expect(page.getByText(/内容变化 1/)).toBeVisible();
  await expect(
    page
      .locator('.generation-list article')
      .getByRole('button', { name: /生\s*成/ }),
  ).toBeVisible();
});

test('polling timeout offers manual refresh and compose failure can retry', async ({
  page,
}) => {
  const { state } = await installFixture(page, 'timeout');
  await login(page);
  ...
  await installFixture(page, 'compose-failure');
  await openWorkflow(page, 'export');
  await page.getByRole('button', { name: '开始合成' }).click();
  await expect(page.getByRole('alert')).toHaveText('mock compose failure');
  await page.getByRole('button', { name: '重试合成' }).click();
  await expect(page.getByText('当前成片')).toBeVisible({ timeout: 10_000 });
});

test('stale final result preserves old preview and requests recomposition', async ({
  page,
}) => {
  await installFixture(page, 'stale-final');
  await login(page);
  await openWorkflow(page, 'export');
...
for (const viewport of [
  ...
  test(`responsive and accessible shell ${viewport.width}x${viewport.height}`, async ({
    page,
  }, testInfo) => {
    await page.setViewportSize(viewport);
    await page.emulateMedia({ reducedMotion: 'reduce' });
    await installFixture(page, 'failed-selection');
    await login(page);
    await openWorkflow(page, 'storyboard');
    expect(
      await page.evaluate(
        () =>
    ...
    if (viewport.width <= 720)
      await expect(page.getByLabel('选择分镜')).toBeVisible();
    ...
    const suffix =
      viewport.width === 1440
        ...
        : viewport.width === 768
          ...
          : 'mobile';
    await page.screenshot({
      path: testInfo.outputPath(`guided-workflow-${suffix}.png`),
      fullPage: true,
    });
  });
}

test('dirty navigation dialog owns focus and Escape cancels navigation', async ({
  page,
}) => {
  await installFixture(page);
  await login(page);
  await openWorkflow(page, 'brief');
  await page.getByLabel('完整剧本').fill('尚未保存的内容');
  await page
    .getByRole('navigation', { name: '视频制作步骤' })
    .getByRole('button', { name: /角色与场景/ })
    .click();
  ...
  await expect(page.locator('[data-step="brief"]')).toBeVisible();
});
