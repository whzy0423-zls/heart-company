    ...
    return (
        <div className={cn("relative overflow-hidden bg-muted dark:bg-white/10", compact ? "min-h-24" : "aspect-[4/3]", className)}>
            <div
                ...
                style={{
                    backgroundImage: "radial-gradient(circle, color-mix(in srgb, var(--primary) 35%, transparent) 1.4px, transparent 1.6px)",
            ...
            <div className="absolute bottom-4 left-4 right-4">
                <div className="mb-2 flex items-center justify-between text-xs text-muted-foreground dark:text-muted-foreground">
                    <span>{formatDuration(tick * 1000)}</span>
                    <span>{Math.floor(progress)}%</span>
                </div>
                <div className="h-1.5 rounded-full bg-muted-foreground/70 dark:bg-white/12">
                    <div className="h-full rounded-full bg-primary dark:bg-muted" style={{ width: `${progress}%` }} />
                </div>
            </div>
        </div>
    );
