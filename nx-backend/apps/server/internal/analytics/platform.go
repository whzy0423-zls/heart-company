...
`, start, now.Add(24*time.Hour)).Scan(&out.Website.TotalUsers, &out.Website.NewUsersToday, &out.Website.ActiveUsersToday, &out.Website.TotalPV, &out.Website.TodayPV, &out.Website.TotalSubmissions, &out.Website.SubmissionsToday, &out.Miniapp.TotalUsers, &out.Miniapp.NewUsersToday, &out.Miniapp.TotalTests, &out.Miniapp.TestsToday, &out.Miniapp.TotalBookings, &out.Miniapp.BookingsToday)
	...
	rows, err := s.db.QueryContext(c, `WITH days AS (SELECT generate_series($1::date,$2::date,'1 day')::date day), w AS (SELECT (create_time AT TIME ZONE 'Asia/Shanghai')::date day,count(DISTINCT COALESCE(NULLIF(visitor_id,''),ip||'|'||user_agent)) n FROM site_visits WHERE create_time >= $1::timestamptz AND create_time < $2::timestamptz GROUP BY 1), u AS (SELECT (create_time AT TIME ZONE 'Asia/Shanghai')::date day,count(*) n FROM wx_users WHERE create_time >= $1::timestamptz AND create_time < $2::timestamptz GROUP BY 1), s AS (SELECT (create_time AT TIME ZONE 'Asia/Shanghai')::date day,count(*) n FROM signups WHERE source_platform='website' AND create_time >= $1::timestamptz AND create_time < $2::timestamptz GROUP BY 1), t AS (SELECT (create_time AT TIME ZONE 'Asia/Shanghai')::date day,count(*) n FROM test_records WHERE create_time >= $1::timestamptz AND create_time < $2::timestamptz GROUP BY 1), b AS (SELECT (create_time AT TIME ZONE 'Asia/Shanghai')::date day,count(*) n FROM bookings WHERE create_time >= $1::timestamptz AND create_time < $2::timestamptz GROUP BY 1) SELECT to_char(days.day,'YYYY-MM-DD'),coalesce(w.n,0),coalesce(u.n,0),coalesce(s.n,0),coalesce(t.n,0),coalesce(b.n,0) FROM days LEFT JOIN w USING(day) LEFT JOIN u USING(day) LEFT JOIN s USING(day) LEFT JOIN t USING(day) LEFT JOIN b USING(day) ORDER BY days.day`, start, now.Add(24*time.Hour))
	if err != nil {
		return out, err
	}
	defer rows.Close()
