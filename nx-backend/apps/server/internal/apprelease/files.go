...
type FileStore struct {
	...
	beforeIconRename     func(string, string)
}
...
func (s *FileStore) SaveIcon(apkKey string, pngData []byte) (string, error) {
	...
	if s.beforeIconRename != nil {
		s.beforeIconRename(tmpPath, destination)
	}
	if err := unix.Renameat(dirfd, tempName, dirfd, destinationName); err != nil {
		unlinkatIfSameFile(dirfd, tempName, tempStat)
