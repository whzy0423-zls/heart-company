...
var (
	...
	chineseIdentityQuestionPatterns = []*regexp.Regexp{
		...
		regexp.MustCompile(`^(当前会话|当前助手|这个会话|这个助手)(调用|使用|运行|通过)(的)?(是)?(什么|啥|哪个|哪一个|哪一版)?(大)?模型$`),
		regexp.MustCompile(`^(告诉我|说一下|透露|展示)?(你|您)的?(底层模型|基座模型|模型(id|编号|版本)|模型参数量|参数量)$`),
		...
		regexp.MustCompile(`^(你|您)的请求(走|通过|使用|调用)(哪个|什么)?(codexcli|中转站|api(代理|中转)|代理api)$`),
