...
func TestOpenAIChatExplicitOutputBudgetUsesProviderPayloadForSyncAndStream(t *testing.T) {
	var budgets []float64
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestOpenAIChatExplicitCompletionTimeoutAppliesToSyncAndStream(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var body map[string]any
	...
	ordinaryGenerator := newTestOpenAIChatGenerator(ordinaryServer)
