...
func (s *Server) appPrivacyPolicy(w http.ResponseWriter, _ *http.Request) {
	httpx.OK(w, appPrivacyPolicyResponse{
		...
		Content: `我们仅为账号登录、九型测评、成长卡片、对话服务、消息推送和服务改进处理必要信息。

使用芯之力语音对话时，服务会临时处理你提交的音频，用于语音识别（ASR）、生成回答和语音合成（TTS）。我们的自有后台不持久化保存原始录音；即使页面不展示文字，后台仍会处理并保存转写文字、AI 回答和回答来源，并可能从对话中提取沟通偏好和对话记忆，用于保持上下文和提供更贴合你的后续回答。
...
你可以在 App 内导出个人数据、清空记忆或注销账号。清空记忆会删除当前账号的对话记忆和沟通偏好。注销后，我们会停用账号、撤销登录凭证，删除聊天、记忆、偏好等 App 数据，将成长卡片标记为删除，对账号标识匿名化，并将必要的分析记录与账号解除关联；法律法规另有保存要求的除外。`,
	})
}
