---
name: realistic-memory-timeline
description: Use when generating a user's “我的故事” in the 时间线记录 style under the realistic story type.
---
...
- Candidate sources: 后端事实卡、用户材料和提纲
...
## Style rules
    ...
    - 先建立事件时间线，再写情绪和回忆，不用叙事效果重排已确认事实。
- 对模糊年份、先后关系和记忆缺口使用明确的不确定表达。
- 同一事件出现多个版本时并列呈现，不替用户裁决未经证实的版本。
- 结尾总结记忆如何变化，不声称回忆必然等同于客观记录。
